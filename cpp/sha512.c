...
#define SHA512_BAD_INPUT_DATA (-2)
...
void sha512_free(sha512_context *ctx)
{
    ...
    platform_zeroize(ctx, sizeof(sha512_context));
}
...
#define P(a, b, c, d, e, f, g, h, x, K)                            \
    ...
    for (i = 0; i < 16; i++)
    {
        local.W[i] = GET_UINT64_BE(data, i << 3);
    }
...
static size_t internal_sha512_process_many(
    sha512_context *ctx, const uint8_t *data, size_t len)
{
    size_t processed = 0;

    while (len >= SHA512_BLOCK_SIZE)
    {
        if (internal_sha512_process(ctx, data) != 0)
        {
            return 0;
        }
                  ...
                  size_t ilen)
{
    ...
    {
        size_t processed =
            internal_sha512_process_many(ctx, input, ilen);
...
static int sha512_common_self_test(int verbose, int is384)
{
    int i, buflen, ret = 0;
    unsigned char *buf;
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        {
            ret = sha512_update(&ctx, sha_test_buf[i],
                                sha_test_buflen[i]);
            if (ret != 0)
            {
                goto fail;
...
{
    return sha512_common_self_test(verbose, 0);
}

int sha384_self_test(int verbose)
{
    return sha512_common_self_test(verbose, 1);
}
