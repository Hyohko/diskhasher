/*
    ...
    from mbedtls_sha256.c of the MBEDTLS cryptography project under
...
#define SHA256_BAD_INPUT_DATA (-2)
#define ERROR_GENERIC_ERROR (-3)
...
void sha256_free(sha256_context *ctx)
{
    ...
    platform_zeroize(ctx, sizeof(sha256_context));
}
...
static size_t internal_sha256_process_many(
    sha256_context *ctx, const uint8_t *data, size_t len)
{
    size_t processed = 0;

    while (len >= SHA256_BLOCK_SIZE)
    {
        if (internal_sha256_process(ctx, data) != 0)
        {
            return 0;
        }
                  ...
                  size_t ilen)
{
    ...
    {
        size_t processed =
            internal_sha256_process_many(ctx, input, ilen);
 ...
 * Checkup routine
 */
static int sha256_common_self_test(int verbose, int is224)
{
    int i, buflen, ret = 0;
    unsigned char *buf;
    ...
    for (i = 0; i < 3; i++)
    {
        if (verbose != 0)
        {
            printf("  SHA-%d test #%d: ", 256 - is224 * 32, i + 1);
        }
        ...
        {
            ret = sha256_update(&ctx, sha_test_buf[i],
                                sha_test_buflen[i]);
            if (ret != 0)
            {
                goto fail;
...
{
    return sha256_common_self_test(verbose, 0);
}

int sha224_self_test(int verbose)
{
    return sha256_common_self_test(verbose, 1);
}